          ...
          {DATA_INTERACTIVE_INTRODUCE.map((item, index) => (
            <Col className="home-interact--card" key={index} md={24} lg={8}>
              <Space direction="vertical">
                <AntImage
                  alt={`${t("home.txt_title_interact")} ${item.title}`}
                  src={staticPath(item.imageUrl)}
                />
                <Typography.Title
                  className="title-1 txt-center mt-24"
                  level={3}
                >
                  {item.title}
                </Typography.Title>
                <Typography.Paragraph className="txt-dec-1 txt-center">
                  {item.description}
